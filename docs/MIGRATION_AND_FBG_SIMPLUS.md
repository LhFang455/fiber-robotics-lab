...
- 公开网站版的上传文件仅在当前运行进程中解析预览，不提供持久化保存。请勿上传未授权公开的 COMSOL、实验或客户数据。
...
在 COMSOL 中沿光纤路径导出空格分隔的 `.txt` 文本。公开教程样例使用八个数值列：
...
先在本网站的 **“FBG-SimPlus 兼容”** 页面上传该文本：页面会检查每行是否为八列数值、是否有非有限值，以及位置是否严格递增，并绘制应变、横向应力和温度预览。该检查不替代 COMSOL 模型验证或 FBG-SimPlus 的参数设置。

### B4. 在 FBG-SimPlus 中生成光谱

启动 `python/run.py` 后，按以下顺序操作：

1. 在 **Select Stressed/Strained Path Files** 区域点击 **Add Files**，选中 COMSOL 导出的 `.txt` 文件。
