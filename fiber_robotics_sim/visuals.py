...
(() => {
 ...
 if(cfg.animate){const motionStart=performance.now(),motionDuration=520,fingerDuration=1000; function tick(now){const progress=Math.min((now-motionStart)/motionDuration,1),eased=1-Math.pow(1-progress,3),fingerProgress=Math.min((now-motionStart)/fingerDuration,1),fingerEased=1-Math.pow(1-fingerProgress,3); shoulder.position.lerpVectors(shoulderStart,shoulderTarget,eased); armPivots.forEach((pivot,index)=>pivot.rotation.z=THREE.MathUtils.lerp(cfg.previousJoints[index],cfg.joints[index],eased)*Math.PI/180); fingerMeshes.forEach((finger,index)=>applyFingerData(finger,cfg.previousFingerCapsules[index],cfg.fingerCapsules[index],fingerEased)); if(!cfg.grasped)can.position.set(cfg.canOffset[0]+canStart.x,cfg.canOffset[1]+canStart.y,cfg.canOffset[2]+canStart.z); frameCamera(); renderer.render(scene,camera);requestAnimationFrame(tick);} requestAnimationFrame(tick);}else{shoulder.position.copy(shoulderTarget);armPivots.forEach((pivot,index)=>pivot.rotation.z=cfg.joints[index]*Math.PI/180);fingerMeshes.forEach((finger,index)=>applyFingerData(finger,cfg.fingerCapsules[index],cfg.fingerCapsules[index],1));if(!cfg.grasped)can.position.set(cfg.canOffset[0]+canStart.x,cfg.canOffset[1]+canStart.y,cfg.canOffset[2]+canStart.z);frameCamera();renderer.render(scene,camera);}
})();</script>'''
