...
当前版本：**v0.3.0** · [版本更新说明](docs/releases/v0.3.0.md)。
...
首页提供“3 分钟看懂光纤感知”，通过足底重心、FBG 温补和稀疏压力重建三章展示成果，可直接进入完整实验。

弯曲标定、材质识别、足底步态、装配校验、连续体形状、结构健康、分布式感知、偏振与干涉、FBG 光学皮肤、动态滑移、三轴触觉单元和稀疏压力重建十二个实验现提供可播放的三维模型，其中光学演示可切换偏振态、Sagnac 环路和 EFPI 微腔三个独立场景。进入对应实验后可重播、暂停、拖动进度、切换速度或全屏讲解；点击“当前参数”与下方实验图表对照。演示可收起，原有参数与导出保留。模型表面和外形有明确示意边界，不代表实测或有限元计算。
